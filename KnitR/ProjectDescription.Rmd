...
#####**In Development**
- **updateTable(series_id,header)** *This will access the LAST_UPDATE table to determine the what data to retrieve from EIA. If no record exists, it will call checkExists and create the table if necessary. Otherwise it will pull all data and call insertRecords() to populate the data.*
